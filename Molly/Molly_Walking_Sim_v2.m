...
duty = 7/8; % (Duty Factor - ratio of number of legs on the ground at a time)
L = 0.030; % m (Stride Length)
leg_lengths = [0, 0.033, 0.038]; % m (Length of each segment of each leg)
liftoff_height = 0.01; % m (Height a leg lifts off of the ground while in swing phase)
body_height = 0.02; % m (Height of body above ground - theoretically remains constant in this setting)
body_l = 0.054; % m
body_w = 0.047; % m
body_h = 0.080; % m
D = leg_lengths(1) + leg_lengths(2) + (body_w/2); % (Distance from foot tip to center of body from top view in standard position) NOT USED ANYWHERE
num_legs = 4;
...
num_contact_legs = 3.5; % NOT USED ANYWHERE

% Q4: Relative Kinematic Phase for Each Leg
phi_start = [1/8, 5/8, 7/8, 3/8]; % start of transfer phase
phi_end = [2/8, 6/8, 0/8, 4/8,]; % end of transfer phase
...
%% Q7:
%{
Calculate positions of all 12 joints at all intervals.
%}
...
leg_joints = [leg1_joints; leg2_joints; leg3_joints; leg4_joints];
...
x_pos = zeros(num_legs, 30);
z_pos = zeros(num_legs, 30);
...
% Leg 3:
x_pos(3,11:30) = x_support_pos(1:20);
x_pos(3,1:10) = x_pos_generic(1:10);
z_pos(3,1:10) = z_pos_generic(1:10);

% Leg 4:
x_pos(4,26:30) = x_support_pos(1:5);
x_pos(4,1:15) = x_support_pos(6:20);
x_pos(4,16:25) = x_pos_generic(1:10);
z_pos(4,16:25) = z_pos_generic(1:10);
...
leg_joints_cycle = [leg1_joints_cycle; leg2_joints_cycle; leg3_joints_cycle; leg4_joints_cycle];
...
%% Q8:
%{
Calculate velocities of all 12 joints at all intervals.
%}
...
if plot_simulation
    ...
    foot(1:12,1) = [hip(1,1) + x_pos(1,1); hip(2,1); (hip(3,1) - z_pos(1,1) - body_height);
                    hip(4,1) + x_pos(2,1); hip(5,1); (hip(6,1) - z_pos(2,1) - body_height);
                    hip(7,1) + x_pos(3,1); hip(8,1); (hip(9,1) - z_pos(3,1) - body_height);
                    hip(10,1) + x_pos(4,1); hip(11,1); (hip(12,1) - z_pos(4,1) - body_height);
    ...
    xlabel('x-axis')
    ylim([-body_w*1.5 body_w*1.5])
    ylabel('y-axis')
    zlim([(-body_height*2) body_height*4])
    zlabel('z-axis')
    ...
    for n = 1:num_cycles
        ...
        for t = 1:size(t_cycle,2)
            ...
            elseif t == 1 && n > 1
                ...
                body_origin(2:3,t) = body_origin(2:3,1); % Preserve y and z data
            else
            ...
            foot(1:12,t) = [hip(1,t) + x_pos(1,t); hip(2,t); (hip(3,t) - z_pos(1,t)) - body_height;
                            hip(4,t) + x_pos(2,t); hip(5,t); (hip(6,t) - z_pos(2,t)) - body_height;
                            hip(7,t) + x_pos(3,t); hip(8,t); (hip(9,t) - z_pos(3,t)) - body_height;
                            hip(10,t) + x_pos(4,t); hip(11,t); (hip(12,t) - z_pos(4,t)) - body_height;
